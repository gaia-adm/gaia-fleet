...
curl -i -X PUT -H "Authorization: token ${GITHUB_API_TOKEN}" -d "{\"path\": \"${service_file}\", \"message\": \"CI build ${build_num}-${branch}\", \"commiter\": {\"name\": \"circleci\", \"email\": \"alexei.led@gmail.com\"}, \"content\": \"${f_64}\", \"sha\": \"${f_sha}\", \"branch\": \"${branch}\"}" https://api.github.com/repos/gaia-adm/gaia-fleet/contents/${service_file}
